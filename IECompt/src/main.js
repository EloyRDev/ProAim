...
var rayTracing = Math.floor(Math.sqrt((Math.pow(Math.abs(mouseY - playerY)) + Math.pow(Math.abs(mouseX - playerX)))));
var bonusActive = [];
...
function start(){
    ...
    reloading = false;
    for(let i = 0; i < bonusSlots.length; i++) bonusSlots[i].ocuped = false;
    for(let i = 0; i < bonusActive.length; i++) { bonusActive[i].remove(); }
    bonusActive = [];
    ...
    for(let i = 0; i < trashBullet.length; i++) trashBullet[i].remove();
    for(let i = 0; i < trashMis.length; i++) trashMis[i].remove();
    for(let i = 0; i < enemies.length; i++) enemies[i].remove();
...
for(let y = 0; y < rangeHeight; y++) {
    ...
    for(let x = 0; x < rangeWidth; x++){
        ...
        for(let i = 0; i < walls.length ; i++){
            x*ms >= walls[i].offsetLeft && x*ms <= walls[i].offsetLeft + walls[i].offsetWidth && y*ms >= walls[i].offsetTop && y*ms <= walls[i].offsetTop + walls[i].offsetHeight ? tempCollisionMap[y][x] = 1 : "";
        }
...
function enemyCollision(b){
    ...
    for(let i = 0; i < enemies.length; i++) { 
        if(!enemies[i].classList.contains("enemy-dead")){
            if(Math.abs(b.offsetLeft - (enemies[i].offsetLeft + 23)) < 23 && Math.abs(b.offsetTop - (enemies[i].offsetTop + 23)) < 23) { 
                if(!enemies[i].hitCounter) {
                    collision = true; 
                    enemies[i].life -= activeGun.damage;
                    enemies[i].life <= 0 ? enemyDamage(enemies[i], true) : enemyDamage(enemies[i], false);
                    enemies[i].life <= 0 ? playerPoints += enemies[i].points : playerPoints += enemies[i].points/4;

                    enemies[i].hitCounter = true;
                    enemyCollisionBugHandler.push(enemies[i]);
                }
            }   
        }
    }
...
function playerCollision(top, left){
    ...
    for(let i = 0; i < bonusActive.length; i++){
        if(Math.floor(Math.sqrt((Math.pow(Math.abs((bonusSlots[bonusActive[i].parent].offsetTop + 25) - playerY)) + Math.pow(Math.abs((bonusSlots[bonusActive[i].parent].offsetLeft + 25) - playerX))))) < 40){
            if(bonusActive[i].type == "gun") activeGun = gunTypes[bonusActive[i].value];
            else if(bonusActive[i].type == "life") playerLife = 1000;
            else if(bonusActive[i].type == "speed") speed = bonusActive[i].value;
            
            bonusActive.splice(bonusActive.indexOf(bonusActive[i]), 1)
            bonusSlots[bonusActive[i].parent].ocuped = false;
            bonusActive[i].remove();
            break;
        }            
    }
...
function shootPriv(){
    ...
    rayTracing = Math.floor(Math.sqrt((Math.pow(Math.abs(mouseY - playerY)) + Math.pow(Math.abs(mouseX - playerX)))));
...
function bulletRay(b){
    ...
    for(let i = 0; i < enemyCollisionBugHandler.length; i++) enemyCollisionBugHandler[i].hitCounter = false;
    ...
    let intern = setInterval(function(){
        ...
        try{
            ...
            else if(activeGun.distance <= Math.floor(Math.sqrt((Math.pow(Math.abs(b.offsetTop - playerY)) + Math.pow(Math.abs(b.offsetLeft - playerX)))))) {
...
function playerMov(){
    ...
    if(!playerMoving && playerLife > 0) {
        ...
        var intern = setInterval(function(){
            ...
            if(!includesKeys(regKeys)) { playerMoving = false; clearInterval(intern); }
        }, 20)
    }
}

function includesKeys(regKeys){
    for(let i = 0; i < regKeys.length; i++)
        if(regKeys[i]) return true;
...
function enemyAI(enem) {
    ...
    let AIProcess = setInterval(function(){
        ...
        if(Math.floor(Math.sqrt((Math.pow(Math.abs((enem.offsetTop+25) - playerY)) + Math.pow(Math.abs((enem.offsetLeft+25) - playerX))))) > 5) {
        ...
        if(Math.floor(Math.sqrt((Math.pow(Math.abs((enem.offsetTop+25) - playerY)) + Math.pow(Math.abs((enem.offsetLeft+25) - playerX))))) < 50){
